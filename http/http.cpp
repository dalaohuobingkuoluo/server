...
HttpMethod StringToHttpMethod(const std::string &v){
#define XX(num, name, method)\
    if(strcmp(#method, v.c_str()) == 0){\
...
HttpMethod CharsToHttpMethod(const char *v){
#define XX(num, name, method)\
    if(strncmp(#method, v, strlen(#method)) == 0){\
        return HttpMethod::name;\
    }
    HTTP_METHOD_MAP(XX)
#undef XX
...
#define XX(num, name, method) #method,
    HTTP_METHOD_MAP(XX)
#undef XX
};

const char* HttpMethodToString(const HttpMethod &v){
    uint32_t idx = (uint32_t)v;
    if(idx >= (sizeof(s_method) / sizeof(s_method[0]))){
        return "<unknow>";
    }
    return s_method[idx].c_str();
}

const char* HttpStatusToString(const HttpStatus &v){
    switch(v){
...
void HttpRequest::delHeader(const std::string& key){
    m_headers.erase(key);           // 实际上请求头支持同一个键有多个值
}
...
HttpResponse::HttpResponse(uint8_t version, bool close)
            : m_version(version), m_status(HttpStatus::OK), m_close(close){
}
