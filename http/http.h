...
#define XX(num, name, method) name = num,
    HTTP_METHOD_MAP(XX) 
#undef XX
    INVALID_METHOD
};
...
public:
  ...
  void getHeaders(const MapType& v) {m_headers = v;}
  void getParams(const MapType& v) {m_params = v;}
  void getCookies(const MapType& v) {m_cookies = v;}
...
private:
  ...
  uint8_t m_version;    //0x11 表示 http1.1
...
public:
  ...
  uint8_t getVetsion() const {return m_version;}
  const std::string& getReason() const {return m_reason;} 
  const std::string& getBody() const {return m_body;}
  const MapType& getHeaders() const {return m_headers;} 

  void setStatus(HttpStatus v) {m_status = v;}
  void setVetsion(uint8_t v) {m_version = v;}
